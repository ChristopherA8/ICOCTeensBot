...
module.exports = {
  ...
  tooQuick: function (amount, interval) {
    ...
    let msg = this.messageLog.filter(
      (log) => log.author == this.message.author.id
    );
  ...
  sameMessages: function (amount, interval) {
    ...
    let msg = this.messageLog.filter(
      (log) => new Date() - log.timeStamp < interval
    );
    ...
    let occurance = (msgContent.match(new RegExp(currentMsg, "g")) || [])
      .length;
